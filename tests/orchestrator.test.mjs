...
describe("orchestrate --eval-mode", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Default runTiers mock — without this, clearAllMocks leaves runTiers
    // returning undefined and the shape test sees an empty results array.
    runTiers.mockResolvedValue({
      results: [
        { tierId: "t0-rtk", tierName: "RTK", severity: "pass", evidence: [], durationMs: 1 },
        { tierId: "t1-env", tierName: "ENV", severity: "info", evidence: [], durationMs: 1 },
        {
          tierId: "t2-cliproxy",
          tierName: "CLIProxy",
          severity: "warn",
          evidence: [{ check: "x", actual: "y", remediation: "z" }],
          durationMs: 1,
        },
        {
          tierId: "t3-cli",
